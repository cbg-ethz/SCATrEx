...
model_args = dict(
    ...
    global_noise_factors_precisions_shape=5.0,
    unobserved_factors_kernel_concentration=1.0 / theta,
...
observed_tree_args = dict(n_nodes=n_clones, node_weights=weights)
...
for node in sim_sca.observed_tree.tree_dict:
    sim_sca.observed_tree.tree_dict[node]["params"] = sim_sca.observed_tree.tree_dict[
        node
    ]["params"][to_keep]
...
np.savetxt(
    simulated_clones_labels,
    np.array(sim_sca.observed_tree.adata.obs["node"]),
    delimiter=",",
    fmt="%s",
)
