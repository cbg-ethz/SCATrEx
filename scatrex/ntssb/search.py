...
class StructureSearch(object):
    ...
    def plot_traces(self, keys=None, figsize=(16,10), highlight_max=True, highlight_s=100, highlight_color='red'):
        ...
        for i, k in enumerate(keys):
            ...
            if highlight_max:
                ax.scatter(it_max_score, self.traces[k][it_max_score], s=highlight_s, color=highlight_color)
                ax.plot([0, it_max_score], [self.traces[k][it_max_score], self.traces[k][it_max_score]], ls='--', color='black')
                if type(self.traces[k][0]) is float or type(self.traces[k][0]) is int or k == 'elbo' or k == 'score':
                    ax.plot([it_max_score, it_max_score], [ax.get_ylim()[0], self.traces[k][it_max_score]], ls='--', color='black')
        plt.show()
    ...
    def pretrain(n_iters_elbo=1000, joint_init=True, thin=10, num_samples=1, step_size=0.001, tol=1e-6, mb_size=100, max_nodes=5, debug=False, callback=None,
        ...
        elbos = self.tree.optimize_elbo(root_node=None, sticks_only=False, num_samples=num_samples, n_iters=n_iters_elbo, thin=thin, tol=tol, step_size=step_size, mb_size=mb_size, max_nodes=max_nodes, init=False, debug=debug, opt=opt, opt_triplet=self.opt_triplet, callback=callback)
        ...
        for i in range(10):
            ...
            self.tree.root['node'].root['node'].variational_parameters['globals']['log_baseline_mean'] = init_log_baseline + np.random.normal(0, 2., size=init_log_baseline.size)
            nodes = self.tree.get_nodes()
            for node in nodes:
                node.variational_parameters['locals']['unobserved_factors_mean'] += np.random.normal(0,.1,size=init_log_baseline.size+1)
            elbos = self.tree.optimize_elbo(root_node=None, sticks_only=False, num_samples=num_samples, n_iters=n_iters_elbo, thin=thin, tol=tol, step_size=step_size, mb_size=mb_size, max_nodes=max_nodes, init=False, debug=debug, opt=opt, opt_triplet=self.opt_triplet, callback=callback)
            scores.append(self.tree.elbo)
            roots.append(deepcopy(self.tree.root))
    ...
    def run_search(self, n_iters=500, n_iters_elbo=100, factor_delay=0, posterior_delay=0, global_delay=0, joint_init=True, thin=10, local=True, num_samples=1, step_size=0.001, verbosity=logging.INFO, tol=1e-6, mb_size=100, max_nodes=5, debug=False, callback=None, alpha=0.0, Tmax=10, anneal=False, restart_step=10,
                    move_weights=None, weighted=True, merge_n_tries=5, opt=adam, search_callback=None, add_rule='accept', add_rule_thres=1., random_seed=1, **callback_kwargs):
        ...
        logger.debug(f'Will search for the maximum marginal likelihood tree with the following moves: {moves}\n')
        ...
        for i in tqdm(range(n_iters)):
            ...
            if move_id == 'add' and self.tree.n_nodes < self.tree.max_nodes-1:
                success, elbos = self.add_node(local=local, num_samples=num_samples, n_iters=n_iters_elbo, thin=thin, step_size=step_size, tol=tol, mb_size=mb_size, max_nodes=max_nodes, debug=debug, opt=opt, weighted=weighted, callback=callback, **callback_kwargs)
            elif move_id == 'merge':
                success, elbos = self.merge_nodes(local=local, num_samples=num_samples, n_iters=n_iters_elbo, thin=thin, step_size=step_size, tol=tol, mb_size=mb_size, max_nodes=max_nodes, debug=debug, opt=opt, callback=callback, **callback_kwargs)
            elif move_id == 'prune_reattach':
                success, elbos = self.prune_reattach(local=local, num_samples=num_samples, n_iters=n_iters_elbo, thin=thin, step_size=step_size, tol=tol, mb_size=mb_size, max_nodes=max_nodes, debug=debug, opt=opt, callback=callback, **callback_kwargs)
            elif move_id == 'pivot_reattach':
                success, elbos = self.pivot_reattach(local=local, num_samples=num_samples, n_iters=n_iters_elbo, thin=thin, step_size=step_size, tol=tol, mb_size=mb_size, max_nodes=max_nodes, debug=debug, opt=opt, callback=callback, **callback_kwargs)
            elif move_id == 'add_reattach_pivot':
                success, elbos = self.add_reattach_pivot(local=local, num_samples=num_samples, n_iters=n_iters_elbo, thin=thin, step_size=step_size, tol=tol, mb_size=mb_size, max_nodes=max_nodes, debug=debug, opt=opt, callback=callback, **callback_kwargs)
            elif move_id == 'swap':
                success, elbos = self.swap_nodes(local=local, num_samples=num_samples, n_iters=n_iters_elbo, thin=thin, step_size=step_size, tol=tol, debug=debug, mb_size=mb_size, max_nodes=max_nodes, opt=opt, callback=callback, **callback_kwargs)
            elif move_id == 'subtree_reattach':
                success, elbos = self.subtree_reattach(local=local, num_samples=num_samples, n_iters=n_iters_elbo, thin=thin, step_size=step_size, tol=tol, debug=debug, mb_size=mb_size, max_nodes=max_nodes, opt=opt, callback=callback, **callback_kwargs)
            elif move_id == 'subtree_pivot_reattach':
                success, elbos = self.subtree_pivot_reattach(local=local, num_samples=num_samples, n_iters=n_iters_elbo, thin=thin, step_size=step_size, tol=tol, debug=debug, mb_size=mb_size, max_nodes=max_nodes, opt=opt, callback=callback, **callback_kwargs)
            elif move_id == 'push_subtree' and self.tree.n_nodes < self.tree.max_nodes-1:
                success, elbos = self.push_subtree(local=local, num_samples=num_samples, n_iters=n_iters_elbo, thin=thin, step_size=step_size, tol=tol, debug=debug, mb_size=mb_size, max_nodes=max_nodes, opt=opt, callback=callback, **callback_kwargs)
            elif move_id == 'extract_pivot' and self.tree.n_nodes < self.tree.max_nodes-1:
                success, elbos = self.extract_pivot(local=local, num_samples=num_samples, n_iters=n_iters_elbo, thin=thin, step_size=step_size, tol=tol, debug=debug, mb_size=mb_size, max_nodes=max_nodes, opt=opt, callback=callback, **callback_kwargs)
            elif move_id == 'optimize_node':
                # Randomly choose a node
                node = np.random.choice(nodes[1:])
                logger.debug(f'Optimizing {node.label}...')
                elbos = self.tree.optimize_elbo(root_node=node, global_only=False, num_samples=num_samples, n_iters=n_iters_elbo, thin=thin, tol=tol, step_size=step_size, mb_size=mb_size, max_nodes=max_nodes, init=False, debug=debug, opt=opt, callback=None, **callback_kwargs)
            elif move_id == 'perturb_node':
                # Randomly choose a node
                node = np.random.choice(nodes[1:])
                logger.debug(f'Perturbing {node.label}...')
                # Perturb a bit and avoid clash between kernel and effect
                perturbation = np.random.normal(0, .5, size=init_log_baseline.size+1)
                node.variational_parameters['locals']['unobserved_factors_kernel_log_mean'] += np.abs(perturbation)
                node.variational_parameters['locals']['unobserved_factors_mean'] += perturbation
                ...
                success, elbos = self.clean_node(local=local, num_samples=num_samples, n_iters=n_iters_elbo, thin=thin, step_size=step_size, tol=tol, debug=debug, mb_size=mb_size, max_nodes=max_nodes, opt=opt, callback=callback, **callback_kwargs)
            elif move_id == 'globals':
                elbos = self.tree.optimize_elbo(root_node=None, global_only=True, num_samples=num_samples, n_iters=n_iters_elbo, thin=thin, tol=tol, step_size=step_size, mb_size=mb_size, max_nodes=max_nodes, init=False, debug=debug, opt=opt, callback=None, **callback_kwargs)
            elif move_id == 'perturb_globals':
                logger.debug("Perturbing globals...")
                # Perturb a bit and optimize all
                perturbation = np.random.normal(0, .5, size=init_log_baseline.size)
                self.tree.root['node'].root['node'].variational_parameters['globals']['log_baseline_mean'] += perturbation
                self.tree.root['node'].root['node'].variational_parameters['locals']['unobserved_factors_mean'] *= 0
                elbos = self.tree.optimize_elbo(root_node=None, global_only=False, num_samples=num_samples, n_iters=n_iters_elbo, thin=thin, tol=tol, step_size=step_size, mb_size=mb_size, max_nodes=max_nodes, init=False, debug=debug, opt=opt, callback=None, **callback_kwargs)
            elif move_id == 'full':
                logger.debug(f'Full update...')
                elbos = self.tree.optimize_elbo(root_node=None, num_samples=num_samples, n_iters=n_iters_elbo, thin=thin, tol=tol, step_size=step_size, mb_size=mb_size, max_nodes=max_nodes, init=False, debug=debug, opt=opt, callback=None, **callback_kwargs)
            ...
            if np.isnan(self.tree.elbo):
                ...
                logger.debug("Proceeding with previous tree, reducing step size and doing `reset_globals`.")
                step_size = step_size * 0.1
                if step_size < 1e-4:
                    logger.debug("Step size is becoming small. Fetching best tree with noise factors.")
                    self.tree.root = deepcopy(self.best_tree.root)
                    self.tree.elbo = self.best_elbo
            ...
            if self.tree.n_nodes >= self.tree.max_nodes:
                ...
                if add_rule == 'accept' and score_type == 'elbo': # only accept immediatly if using ELBO to score
                    logger.debug(f'*Move ({move_id}) accepted. ({init_elbo} -> {self.tree.elbo})*')
            ...
            if i == factor_delay and n_factors > 0:
                logger.debug("Setting current tree with complete number of factors as the best.")
                self.best_elbo = self.tree.elbo
                self.best_tree = deepcopy(self.tree)
                logger.debug(f'New best! {self.best_elbo}')
    ...
    def add_node(self, local=False, num_samples=1, n_iters=100, thin=10, tol=1e-7, step_size=0.05, mb_size=100, max_nodes=5, debug=False, opt=None, weighted=True, callback=None, **callback_kwargs):
        success = True
        elbos = []

        nodes, target_probs = self.tree.get_node_data_sizes(normalized=True)
        if not weighted:
            target_probs = np.array([1.] * len(nodes))
        ...
        unobs_children = [child for child in list(node.children()) if not child.is_observed and child != new_node]
        ...
        if from_factor:
            ...
            elbos = self.tree.optimize_elbo(local_node=None, root_node=None, num_samples=num_samples, n_iters=2*n_iters, thin=thin, tol=tol, step_size=step_size, mb_size=mb_size, max_nodes=max_nodes, init=False, debug=debug, opt=opt, opt_triplet=self.opt_triplet, callback=callback, **callback_kwargs)
        else:
            if node.parent() is None: # if root, need to update also global factors
                elbos = self.tree.optimize_elbo(local_node=None, root_node=None, num_samples=num_samples, n_iters=n_iters*5, thin=thin, tol=tol, step_size=step_size, mb_size=mb_size, max_nodes=max_nodes, init=False, debug=debug, opt=opt, opt_triplet=self.opt_triplet, callback=callback, **callback_kwargs)
            else:
                elbos = self.tree.optimize_elbo(local_node=local_node, root_node=root_node, num_samples=num_samples, n_iters=n_iters, thin=thin, tol=tol, step_size=step_size, mb_size=mb_size, max_nodes=max_nodes, init=False, debug=debug, opt=opt, opt_triplet=self.opt_triplet, callback=callback, **callback_kwargs)

        return success, elbos

    def merge_nodes(self, local=False, num_samples=1, n_iters=100, thin=10, tol=1e-7, step_size=0.05, mb_size=100, max_nodes=5, debug=False, opt=None, callback=None, **callback_kwargs):
        ...
        if np.any(n_nodes > 1):
            ...
            elbos = self.tree.optimize_elbo(unique_node=None, root_node=local_node, run=True, num_samples=num_samples, n_iters=n_iters, thin=thin, tol=tol, step_size=step_size, mb_size=mb_size, max_nodes=max_nodes, init=False, debug=debug, opt=opt, opt_triplet=self.opt_triplet, callback=callback, **callback_kwargs)

        return success, elbos

    def prune_reattach(self, local=False, num_samples=1, n_iters=100, thin=10, tol=1e-7, step_size=0.05, mb_size=100, max_nodes=5, debug=False, opt=None, callback=None, **callback_kwargs):
        success = False
        elbos = []
    ...
    def pivot_reattach(self, local=False, num_samples=1, n_iters=100, thin=10, tol=1e-7, step_size=0.05, mb_size=100, max_nodes=5, debug=False, opt=None, callback=None, **callback_kwargs):
        success = False
        elbos = []
        ...
        if len(nodes) > 1:
            ...
            if len(init_pivot_node.data) == 0 and init_pivot_node_parent is not None and len(init_pivot_node.children()) == 0:
                logger.debug(f"Also removing initial pivot ({init_pivot_node.label}) from tree")
            ...
            elbos = self.tree.optimize_elbo(root_node=root_node, num_samples=num_samples, n_iters=n_iters, thin=thin, tol=tol, step_size=step_size, mb_size=mb_size, max_nodes=max_nodes, init=False, debug=debug, opt=opt, opt_triplet=self.opt_triplet, callback=callback, **callback_kwargs)


        return success, elbos

    def add_reattach_pivot(self, local=False, num_samples=1, n_iters=100, thin=10, tol=1e-7, step_size=0.05, mb_size=100, max_nodes=5, debug=False, opt=None, callback=None, **callback_kwargs):
        success = True
        elbos = []
        ...
        logger.debug(f"Trying to add node {pivot_node.label} and setting it as pivot of {subtree['node'].label}")
        ...
        if pivot_node.parent().parent() is None:
            ...
            elbos = self.tree.optimize_elbo(local_node=pivot_node, root_node=None, num_samples=num_samples, n_iters=n_iters, thin=thin, tol=tol, step_size=step_size, mb_size=mb_size, max_nodes=max_nodes, init=False, debug=debug, opt=opt, opt_triplet=self.opt_triplet, callback=callback, **callback_kwargs)
        elbos = self.tree.optimize_elbo(root_node=root_node, num_samples=num_samples, n_iters=n_iters_elbo, thin=thin, tol=tol, step_size=step_size, mb_size=mb_size, max_nodes=max_nodes, init=False, debug=debug, opt=opt, opt_triplet=self.opt_triplet, callback=callback, **callback_kwargs)


        return success, elbos

    def push_subtree(self, local=False, num_samples=1, n_iters=100, thin=10, tol=1e-7, step_size=0.05, mb_size=100, max_nodes=5, debug=False, opt=None, callback=None, **callback_kwargs):
        success = True
        elbos = []
    ...
    def extract_pivot(self, local=False, num_samples=1, n_iters=100, thin=10, tol=1e-7, step_size=0.05, mb_size=100, max_nodes=5, debug=False, opt=None, callback=None, **callback_kwargs):
        success = True
        elbos = []
    ...
    def subtree_reattach(self, local=False, num_samples=1, n_iters=100, thin=10, tol=1e-7, step_size=0.05, mb_size=100, max_nodes=5, debug=False, opt=None, callback=None, **callback_kwargs):
        """
        ...
        if len(in_subtrees) > 0:
            ...
            logger.debug(f"Trying to set {roots[nodeA_idx]['node'].label} below {new_subtree['node'].label}")

            # Move subtree
            optimal_init = bool(np.random.binomial(1, 0.5))
            pivot_changed = self.tree.subtree_reattach_to(roots[nodeA_idx]['node'], new_subtree['node'], optimal_init=optimal_init)

            # Also swap to make the moved subtree root be the new root?
            if len(list(roots[nodeA_idx]['node'].children())) == 0:
                if np.random.binomial(1, 0.5):
                    self.tree.swap_nodes(roots[nodeA_idx]['node'], new_subtree['node'].root['node'])
            ...
            elbos = self.tree.optimize_elbo(root_node=root_node, num_samples=num_samples, n_iters=n_iters*5, thin=thin, tol=tol, step_size=step_size, mb_size=mb_size, max_nodes=max_nodes, init=False, debug=debug, opt=opt, opt_triplet=self.opt_triplet, callback=callback, **callback_kwargs)



        return success, elbos

    def subtree_pivot_reattach(self, local=False, num_samples=1, n_iters=100, thin=10, tol=1e-7, step_size=0.05, mb_size=100, max_nodes=5, debug=False, opt=None, callback=None, **callback_kwargs):
        """
        ...
        if len(in_subtrees) > 0:
            ...
            logger.debug(f"Trying to set {roots[nodeA_idx]['node'].label} below {subtreeA['super_parent'].label} and use it as pivot of {subtreeA['node'].label}")
    ...
    def swap_nodes(self, local=False, num_samples=1, n_iters=100, thin=10, tol=1e-7, step_size=0.05, mb_size=100, max_nodes=5, debug=False, opt=None, callback=None, **callback_kwargs):
        success = True
        elbos = []
        ...
        def descend(root, subtree, ntree, done):
            if not done:
                if root['node'] == subtree:
                    tssb_swap(subtree, root['children'], ntree, n_iters, update_pivots=update_pivots)
                    return True
                else:
        ...
        if within_tssb:
            ...
            if nodeB is not None:
                ...
                if unrestricted:
                    if nodeA == nodeB.parent():
                        ...
                        for child in root_node.children():
                            child.variational_parameters['locals']['unobserved_factors_kernel_log_mean'] = np.clip(root_node.variational_parameters['locals']['unobserved_factors_kernel_log_mean'], -3, 10)
                ...
                if not local:
                    root_node = None
                elbos = self.tree.optimize_elbo(root_node=root_node, num_samples=num_samples, n_iters=new_n_iters, thin=thin, tol=tol, step_size=step_size, mb_size=mb_size, max_nodes=max_nodes, init=False, debug=debug, opt=opt, opt_triplet=self.opt_triplet, callback=callback, **callback_kwargs)


        return success, elbos

    def perturb_node(self, local=False, num_samples=1, n_iters=100, thin=10, tol=1e-7, step_size=0.05, mb_size=100, max_nodes=5, debug=False, opt=None, callback=None, **callback_kwargs):
    ...
    def clean_node(self, local=False, num_samples=1, n_iters=100, thin=10, tol=1e-7, step_size=0.05, mb_size=100, max_nodes=5, debug=False, opt=None, callback=None, **callback_kwargs):
