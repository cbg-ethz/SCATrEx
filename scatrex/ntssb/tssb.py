...
class TSSB(object):
    ...
    def __init__(self, root_node, label, ntssb=None, dp_alpha=1.0, dp_gamma=1.0, min_depth=0, max_depth=15, alpha_decay=0.5, eta=0., color='black'):
        ...
        self.root        = { 'node'     : root_node,
                             'main'     : boundbeta(1.0, dp_alpha) if self.min_depth == 0 else 0.0, # if min_depth > 0, no data can be added to the root (main stick is nu)
    ...
    def reset_node_parameters(self, root_params=True, down_params=True, node_hyperparams=None):
        ...
        def descend(root):
            root['node'].reset_parameters(root_params=root_params, down_params=down_params, **node_hyperparams)
            for child in root['children']:
                descend(child)
        descend(self.root)
    ...
    def truncate(self):
        ...
        def descend(root, depth=0):
            for i, child in enumerate(root['children']):
                root['sticks'][i] = 1. if i == len(root['children']) - 1 else root['sticks'][i]
    ...
    def resample_node_params(self, iters=1, independent_subtrees=False, top_node=None, data_indices=None, compound=True):
        """
    ...
    def resample_tree_topology(self, children_trees, independent_subtrees=False):
        ...
        if len(nodes)>1:
            ...
            def swap_nodes(nodeAnum, nodeBnum, verbose=False):
                ...
                nodeA['node'].node_mean = nodeA['node'].baseline_caller() * nodeA['node'].cnvs/2 * np.exp(nodeA['node'].unobserved_factors)
                ...
                nodeB['node'].node_mean = nodeB['node'].baseline_caller() * nodeB['node'].cnvs/2 * np.exp(nodeB['node'].unobserved_factors)
                ...
                if not independent_subtrees:
                    ...
                    for child in children_trees:
                        if child['pivot_node'] == nodeA['node']:
                            ...
                            child['node'].root['node'].set_parent(nodeB['node'], reset=False)
    ...
    def cull_tree(self, verbose=False, resample_sticks=True):
        ...
        def descend(root):
            ...
            s = 0
            if len(list(root['node'].children())) > 0: # make sure we only delete leaves
                s = 1
    ...
    def sample_main_sticks(self, truncate):
        def descend(root, depth=0):
            main = boundbeta(1.0, (self.alpha_decay**(depth+1))*self.dp_alpha) if self.min_depth <= (depth+1) else 0.0
            root['main'] = 1. if len(root['children']) < 1 and truncate else main
            for i, child in enumerate(root['children']):
                descend(child, depth+1)
        descend(self.root)
    ...
    def sample_sticks(self, truncate, balanced):
        def descend(root, depth=0):
            for i, child in enumerate(root['children']):
                ...
                if balanced:
                    stick = 1./(len(root['children']) - i)
                root['sticks'][i] = 1.0 if i == len(root['children']) - 1 and truncate else stick
                descend(child, depth+1)
        descend(self.root)
    ...
    def resample_sticks(self, top_node=None, prior=False, truncate=False):
        def descend(root, depth=0):
            ...
            for i in indices:
                ...
                if prior:
                    root['sticks'][i] = boundbeta(1., self.dp_gamma) # Updates psi-sticks
                else:
                    root['sticks'][i] = boundbeta(post_alpha, post_beta) # Updates psi-sticks
            ...
            if prior:
                root['main'] = boundbeta(1., (self.alpha_decay**depth)*self.dp_alpha) if self.min_depth <= depth else 0.0 # Updates nu-sticks
            else:
                root['main'] = boundbeta( post_alpha, post_beta ) if self.min_depth <= depth else 0.0 # Updates nu-sticks
    ...
    def _resample_stick_orders(self):
        ...
        def descend(root, depth=0):
            ...
            for k in list(filter(lambda k: k not in new_order, range(root['sticks'].shape[0]))):
    ...
    def resample_stick_orders(self):
        ...
        def descend(root, depth=0):
            if len(root['children']) == 0: # Only nodes with children contain  psi-sticks
                return
            ...
            root['children'] = new_children
            root['sticks']   = np.array(new_sticks).reshape(-1, 1) #zeros((len(root['children']),1))
        descend(self.root)
    ...
    def resample_hypers(self, dp_alpha=True, alpha_decay=True, dp_gamma=True):
        ...
        """
        def dp_alpha_llh(dp_alpha, alpha_decay): # Don't consider the root on these computations, nor the subclusters
            ...
            def descend(dp_gamma, root):
                ...
                if root['node'].parent() is None: # only update root's children
                    for i, child in enumerate(root['children']):
                        ...
                        if root['node'].parent() is None and (i < len(root['children'])-1):
    ...
    def get_expected_mixture(self, reset_names=False):
        ...
        def descend(root, mass, depth=0):
            weight  = [ mass * (1.0 / (1.0 + self.dp_alpha*self.alpha_decay**(depth+1) )) if len(root['children']) != 0 else mass ]
            node    = [ root['node'] ]
            sticks = [1./(1. + self.dp_gamma) for stick in root['sticks']]
            ...
            for i, child in enumerate(root['children']):
                (child_weights, child_nodes) = descend(child, mass*(1.0 - 1.0/(1.0 + self.dp_alpha*self.alpha_decay**(depth+1)))*weights[i], depth+1)
                weight.extend(child_weights)
                node.extend(child_nodes)
            return (weight, node)
        return descend(self.root, 1.0)

    def get_mixture(self, reset_names=False, get_roots=False, get_depths=False, truncate=False):
        """
        ...
        def descend(root, mass, depth=0):
            main    = 1. if len(root['children']) < 1 and truncate else root['main']
            weight  = [ mass * main ]
            node    = [ root['node'] ]
            roots   = [root]
            depths  = [depth]
            sticks  = np.array([1. if i == len(root['children']) - 1 and truncate else root['sticks'][i] for i in range(len(root['children']))]).astype(float)
            ...
            for i, child in enumerate(root['children']):
                if get_roots:
                    if get_depths:
                        (child_weights, child_nodes, child_roots, child_depths) = descend(child, mass*(1.0-root['main'])*weights[i], depth=depth+1)
                    else:
                        (child_weights, child_nodes, child_roots) = descend(child, mass*(1.0-root['main'])*weights[i])
                else:
    ...
    def remove_empty_nodes(self):
        ...
        def descend(root):
            ...
            while True:
                ...
                empty_nodes = list(filter(lambda i:
    ...
    def subsample_tree(self, p=0.5):
        root_dict = dict()
        root_dict[self.root['node'].label] = dict(parent='-1',
        ...
        def descend(root, parent):
            for i, child in enumerate(root['children']):
                ...
                pa = root['node'].label
                if keep:
                    root_dict[child['node'].label] = dict(parent=parent,
