...
class Node(AbstractNode):
    ...
    def reset_parameters(self, root_params=True, down_params=True,
        ...
        if parent is None: # this is the root
            ...
            if root_params:
                ...
                self.baseline = np.random.gamma(self.baseline_shape, 1, size=self.n_genes)
            ...
            if down_params:
                ...
                if not self.is_observed:
                    ...
                    self.unobserved_factors_kernel[top_genes] = np.max([5., np.max(self.unobserved_factors_kernel)])
                ...
                if not self.is_observed:
                    self.unobserved_factors[top_genes] = MAX_XI # just force an amplification
    ...
    def loglh(self, n, variational=False, axis=None):
        ...
        return jax.partial(jit, static_argnums=2)(poisson_lpmf)(jnp.array(self.tssb.ntssb.data[n]),  lib_sizes * node_mean, axis=axis)
    ...
    def _compute_elbo(self, rng, cnvs, parent_vector, children_vector, ancestor_nodes_indices, tssb_indices, previous_branches_indices, tssb_weights, dp_alphas, dp_gammas, node_mask, data_mask_subset, indices, do_global, global_only, sticks_only,
                nu_sticks_log_means, nu_sticks_log_stds,
                psi_sticks_log_means, psi_sticks_log_stds, unobserved_means, unobserved_log_stds, log_unobserved_factors_kernel_means, log_unobserved_factors_kernel_log_stds,
                log_baseline_mean, log_baseline_log_std, cell_noise_mean, cell_noise_log_std, noise_factors_mean, noise_factors_log_std, factor_precision_log_means, factor_precision_log_stds):
        ...
        factor_precision_log_stds = jnp.clip(factor_precision_log_stds, a_min=jnp.log(1e-2), a_max=jnp.log(1e2))
        log_factors_precisions = diag_gaussian_sample(rng, factor_precision_log_means, factor_precision_log_stds)
        noise_factors_mean = jnp.clip(noise_factors_mean, a_min=-10., a_max=10.)
        noise_factors_log_std = jnp.clip(noise_factors_log_std, a_min=jnp.log(1e-2), a_max=jnp.log(1e2))
        noise_factors = diag_gaussian_sample(rng, noise_factors_mean, noise_factors_log_std)
        cell_noise_mean = jnp.clip(cell_noise_mean, a_min=-10., a_max=10.)
        cell_noise_log_std = jnp.clip(cell_noise_log_std, a_min=jnp.log(1e-2), a_max=jnp.log(1e2))
