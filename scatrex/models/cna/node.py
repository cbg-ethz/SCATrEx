    ...
    def _compute_elbo(self, rng, cnvs, parent_vector, children_vector, ancestor_nodes_indices, tssb_indices, previous_branches_indices, tssb_weights, dp_alphas, dp_gammas, node_mask, data_mask_subset, indices, do_global, global_only, sticks_only,
        ...
        def compute_node_kl(i):
            ...
            pl = diag_gamma_logpdf(1e-2 * np.ones(broadcasted_concentration.shape), broadcasted_concentration,
                                    (parent_vector[i] != -1)*jnp.abs(nodes_unobserved_factors[parent_vector[i]]))
            ent = - diag_gaussian_logpdf(jnp.clip(nodes_log_unobserved_factors_kernels[i], a_min=jnp.log(1e-2)), jnp.log(1e-2) * np.ones(broadcasted_concentration.shape), log_unobserved_factors_kernel_log_stds[i])
